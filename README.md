...
| Audio recitations | [EveryAyah.com](https://everyayah.com/) ([recitations index](https://everyayah.com/recitations_ayat.html)) | Per-reciter terms; attribution required. We construct verse-level URL paths only — no audio is bundled or cached. Slugs verified against the live CDN; see `src/data/audioReciters.json` `_meta.verifiedAt`. EveryAyah hosts Ḥafṣ (Husary, Alafasy, Maher al-Muaiqly, Abdul Basit Murattal) + Warsh (Abdul Basit, Ibrahim Al-Dosary, Yassin Al-Jazaery); **no Qālūn reciter** — panel renders "audio not available" for Qālūn rows. QUL's Ali al-Huthaify Qālūn is documented as future-work in the manifest. |
