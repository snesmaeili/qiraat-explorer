...
- **Phase 3 — audio (partially started).** [`src/data/audioReciters.json`](../src/data/audioReciters.json) lists 7 EveryAyah-hosted reciters (4 Ḥafṣ: Husary, Alafasy, Maher al-Muaiqly, Abdul Basit Murattal; 3 Warsh: Abdul Basit, Ibrahim Al-Dosary, Yassin Al-Jazaery). All slug paths are HTTP-200-verified against the live CDN (`_meta.verifiedAt`). [`src/lib/audio.js`](../src/lib/audio.js) constructs EveryAyah URLs (`https://everyayah.com/data/{slug}/{paddedSurah}{paddedAyah}.mp3`); per-reading audio play buttons in `VariantPanel.jsx` lazy-instantiate an HTMLAudioElement on click. **EveryAyah does not host any Qālūn reciter** — the Qālūn row renders "audio not available". QUL's Ali al-Huthaify Qālūn (132+ reciters, ayah-level timing, JS-gated downloads) is documented for future ingestion, but bypassing the JS gate is non-trivial and was deferred.
