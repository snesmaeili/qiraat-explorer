...
- **Phase 3 — audio (partially started).** [`src/data/audioReciters.json`](../src/data/audioReciters.json) lists 10 reciters across two CDNs:
  - **EveryAyah (verse-level, granularity `ayah`)** — 4 Ḥafṣ (Husary, Alafasy, Maher al-Muaiqly, Abdul Basit Murattal) + 3 Warsh (Abdul Basit, Ibrahim Al-Dosary, Yassin Al-Jazaery). Each verse is a separate ~50-500 KB MP3.
  - **MP3Quran (surah-level, granularity `surah`)** — 3 Qālūn reciters (Mahmoud Khalil al-Husary, Marwan Alakri, Muhammad Abu Sneina) sourced via MP3Quran's [API v3](https://mp3quran.net/api/v3/reciters?language=en). One MP3 per chapter (multi-MB); the audio button shows a dashed border and explanatory tooltip noting "plays from the start of surah N, not just verse X." This is honest about the UX limitation while filling the Qālūn gap.

  All paths HTTP-200-verified against the live CDNs (`_meta.verifiedAt`). [`src/lib/audio.js`](../src/lib/audio.js) returns `{url, granularity}` for each reciter; per-reading audio play buttons in `VariantPanel.jsx` lazy-instantiate an HTMLAudioElement on click and clean up on unmount. Riwāyas with no configured reciter (Duri, Susi, Bazzi, etc.) render the disabled "audio not available" affordance. QUL's Ali al-Huthaify Qālūn (verse-level timing behind a JS gate) remains an option for future verse-level Qālūn integration.
